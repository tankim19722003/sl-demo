...
if ($total_pages > 1) {
	...
	// 1. Manually add disabled "Prev" button if on the first page
	if ($current_page == 1) {
	...
	// 2. Loop through default WordPress pagination links
	if ($pagination) {
		foreach ($pagination as $page_link) {
			$class = str_contains($page_link, 'current') ? 'page-item active-primary ' : 'page-item';
	...
	// 3. Manually add disabled "Next" button if on the last page
